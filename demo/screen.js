...
Screen.prototype.dim = function(app, dimmed, t, del) {
    ...
    for(let i in this.works) {
        ...
        if(this.works[i].img && this.works[i].img.mat) {
            let tw = new TWEEN.Tween(this.works[i].main.mat)
                .easing(TWEEN.Easing.Exponential.Out)
                .delay(d)
                .to({
                    opacity: op
...
Artistscreen.prototype.zoom = function(app, zoomed, i, t, del) {
    ...
    let fov;
    if(!app.mobile) fov = zoomed ? 34 : 50;
    else fov = zoomed ? 40 : 50;
...
export function Homescreen(app) {
    ...
    for(let k in app.data.artists) {
        ...
        this.works[i] = new Cover(app, v.dir, v.cover, v.name, place(this, app, i));
